...
double solution_nonlineaire_elliptique_sample(double mu, double a, double e, std::vector<double> x,double S)
{
    int ifault;
    double W = law_gamma(mu,1.0);
    double U = law_uniform(0.0,1.0);
    int siz = d;
    std::vector<double> y = x;
    double dt=W/200/(1+0.5*sqrt(norm2(x))); 
    double t=0;
    while (t<W)
      {
	if (norm2(y) > 1)
    ...
    if (U < 0.5)
    {
        return 2*S*(sol_funct(a,y)-pow(condition_initiale(a, y),2))/gamma_pdf(W,mu);
    }
    else
    {
        double u1 = solution_nonlineaire_elliptique_sample(mu, a, e, y, 1);
        return solution_nonlineaire_elliptique_sample(mu, a,e, y, 2*u1*S/gamma_pdf(W,mu));
    }

}

double solution_nonlineaire_elliptique(double mu, double a, double e, std::vector<double> x, int n)
{
    double result = 0;
    double v;
    int m=n;
    for (int j=0; j<(n/m);j++)
      ...
      for (int i=0; i <m;i++)
	  {
	    double samp = std::clamp(solution_nonlineaire_elliptique_sample(mu, a, e, x, 1), -5000.0, 5000.0);
	result += samp;
    }
      printf("%d\t%f\t%f\n",(j+1)*m,x[0],result/((j+1)*m));
}
    printf("result=%f\n",result/n);
 return (result/n);
...
int main() {
    ...
    double mu = 1.1;
    double e = 0.0;
    const double constant_factor = 1.0; 
    ...
    for (int k = 0; k <= 10; ++k) {
        ...
        std::printf("x=%f\n", x[0]);
        int N = 20000000;
	double est = solution_nonlineaire_elliptique(mu,alpha,e,x,N); 
        std::printf("y=%f\n", est);
