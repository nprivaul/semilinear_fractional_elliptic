...
double solution_lineaire_elliptique_exp_sample(double mu, double a, double e, std::vector<double> x,double S)
{
    if (norm2(x)>1)
    {
        return(0.0);
    ...
    if (U < 0.5)
    {
        return 2*exp(-W)*S*(sol_funct(a,y)-pow(condition_initiale(a, y),1))/gamma_pdf(W,mu);
    }
    else
    {
        return solution_lineaire_elliptique_exp_sample(mu, a,e, y, 4*exp(-W)*S/gamma_pdf(W,mu));
    }
}

double solution_lineaire_elliptique_exp(double mu, double a, double e, std::vector<double> x, int n)
{
     double result = 0;
    double v;
    int m=1;
      for (int i=0; i <n;i++)
	  {
	    double samp = std::clamp(solution_lineaire_elliptique_exp_sample(mu, a, e, x, 1), -5000.0, 5000.0);
	result += samp;
    }
 return (result/n);
...
int main() {
    ...
    for (int k = 0; k <= 10; ++k) {
        ...
        std::printf("x=%f\n", x[0]);
        int N = 5000000;
	double est = solution_lineaire_elliptique_exp(mu,alpha,e,x,N); 
        std::printf("y=%f\n", est);
